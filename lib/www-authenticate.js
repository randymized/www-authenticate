...
module.exports = function(username,password,options)
{
  ...
  HigherLevel.prototype.get_challenge= function(request) {
    if (401 == request.statusCode && 'www-authenticate' in request.headers)
      this.challenge= parse_header(request.headers['www-authenticate'])
      return this.challenge.err;
  }
  HigherLevel.prototype.authenticate= function(headers,method,digestURI) {
    if (!this.challenge) return;   // simply won't produce an 'Authorization' header
    if (this.challenge.err) return this.challenge.err;
    headers.authorization= this.challenge.authorize(method,digestURI);
  }
  HigherLevel.prototype.authenticate_request_options= function(request_options) {
    ...
    request_options.headers.authorization= this.challenge.authorize(request_options.method,request_options.path);
  }
  parse_header.authenticator= new HigherLevel();
  return parse_header;
};
